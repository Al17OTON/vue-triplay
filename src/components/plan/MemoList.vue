...
<template>
    <div v-show="inputId===0" id="commentDiv">
    ...
    <div id="comment-area">
        <div v-for="memo in memoList" :key="memo.memo_id" :style="{'margin' : '0 0 0 ' + (memo.depth * 5) + '%'}">
            ...
            <div v-show="inputId===memo.memo_id" id="commentDiv" class="input-group mb-3">
                <!-- 댓글 작성 Form -->
                <input v-model="commentTable['id'+memo.memo_id]" id="commentArea" class="form-control" type="text" placeholder="댓 글" required />
                <button class="float-end btn btn-outline-success mb-3 ms-1 mt-1 " type="button" @click="submitMemo(memo)">등록</button>
            </div>
            <div :style="{'claer' : 'both'}"></div>
        </div>
    </div>
